...
"""
測試水庫基本資料 API
API 端點: https://opendata.wra.gov.tw/Service/OpenData.aspx?format=json&id=D54BA676-ED9A-4077-9A10-A0971B3B020C
"""
...
async def test_reservoir_basic_info_api():
    """測試水庫基本資料 API"""
    print("📋 測試水庫基本資料 API...")
    print("=" * 50)
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=30)) as response:
                if response.status == 200:
                    ...
                    filename = f"reservoir_basic_info_{timestamp}.json"
...
if __name__ == "__main__":
    success = asyncio.run(test_reservoir_basic_info_api())
    
    if success:
        print("\n🎯 下一步:")
        print("  ✅ 水庫基本資料 API 分析完成")
